    ...
    case "${args}" in
        -)
            case ${OPTARG} in
                ...
                Help) usage ;;
                *)
                    echo "[ FAILED ]\tError in command line parsing.  Incorrect switch used." >&2
                    exit 1
            esac;;
        h)
            usage ;;
        help)
            usage ;;
        *)
            echo "[ FAILED ]\tError in second command line parsing (2 tacks?)" >&2
            exit 1
    esac
done
...
if (($EUID != 0)); then
    echo "[ FAILED ]\tMust be run with sudo privileges."
    exit 1
fi
...
if [[ ! -z "$MOUNT_MODE" ]]; then
        echo "[ FAILED ]\tMust be run from a partition mounted without 'noexec'"
        exit 1
fi
...
if [ "$DownloadPS" = "true" ]; then
    ...
    if [[ $Proxy = "" ]]; then
        if ! curl --retry 5 --retry-delay 5 -L -o $ESPATH/powershell.tar.gz $ARTIFACT_URL; then
            echo "[ FAILED ]\tPowerShell archive failed to download."
            exit 1
        fi
    else
...
if (( $(echo "$CURRENT_VERSION < $VERSION_LIMIT" | bc -l) )); then
    echo "[ FAILED ]\tKernel version not supported. "
    exit 1
fi
...
if [[ $PWSHPath = "" ]]; then
    if [[ $PSPath = "" ]]; then
        ...
        if [ -z "$POWERSHELL" ]; then
            echo "[ FAILED ]\tPrerequisite failed.  PowerShell binary archive not found in $ESPATH."
            echo "[ ALERT ]Use the '--DownloadPS' switch or download tar.gz archive, rename to 'powershell.tar.gz' and place in $ESPATH."
            echo "[ ALERT ]https://learn.microsoft.com/en-us/powershell/scripting/install/install-other-linux?view=powershell-7.4#binary-archives"
            exit 1
        fi
        ...
        if [ -d $PSPath ]; then
            if [[ -z $(find -H $PSPath -name pwsh) ]]; then
                echo "[ FAILED ]\tPrerequisite failed.  PowerShell executable not found in $PSPath."
                exit 1
            else
                PSEXECPATH=$PSPath
            fi
        else
            echo "[ FAILED ]\tPrerequisite failed. $PSPath is not a valid directory."
            exit 1
        fi
    fi
